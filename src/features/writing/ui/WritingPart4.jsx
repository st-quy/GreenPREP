...
import { useWritingPart4 } from '../hooks/useWritingPart4';
...
const WritingPart4 = () => {
  const { data, isLoading, error } = useWritingPart4();
  const dispatch = useDispatch();

  useEffect(() => {
    // Set initial time to 30 minutes (1800 seconds) for both questions
    dispatch(setTime(1800));
  }, [dispatch]);
      ...
      <div className="flex flex-col">
        ...
        <div className="w-full lg:w-full bg-[#FFFFFF] rounded-lg p-6 inline-block " style={{ border: '0.5px solid rgba(0, 0, 0, 0.3)' }}>
          <div className="flex justify-between items-center mb-4">
            <h2><span className="text-[#3758F9]">Part 4</span> of 4</h2>
            <MarkerButton onClick={() => console.log('Marked!')} />
          </div>

          <div className="space-y-4">
            <div className="text-lg">{data.content}</div>
            <div className="text-sm text-gray-600 whitespace-pre-line">{data.subContent}</div>
