...
export function useFullScreen() {
  ...
  // Use a ref to track if event listeners are set up
  const eventListenersSetUp = useRef(false);

  // Ref to track if a modal or message is already being displayed
  const modalShownRef = useRef(false);
  const messageShownRef = useRef(false);

  // Function to show a modal safely (prevents spamming)
  const showModal = useCallback((modalConfig) => {
    if (modalShownRef.current) return; // Prevent showing multiple modals
    modalShownRef.current = true;

    Modal.warning({
      ...modalConfig,
      onOk: () => {
        modalShownRef.current = false; // Reset the flag when modal is dismissed
        if (modalConfig.onOk) modalConfig.onOk();
      },
      onCancel: () => {
        modalShownRef.current = false; // Reset the flag when modal is canceled
        if (modalConfig.onCancel) modalConfig.onCancel();
      },
    });
  }, []);

  // Function to show a message safely (prevents spamming)
  const showMessage = useCallback((messageConfig) => {
    if (messageShownRef.current) return; // Prevent showing multiple messages
    messageShownRef.current = true;

    message[messageConfig.type || "info"]({
      ...messageConfig,
      onClose: () => {
        messageShownRef.current = false; // Reset the flag when message is closed
  ...
  }, [browserInfo?.supportsFullScreen, detectionFailed]);

  // Set up event listeners for keyboard, right-click, etc.
  const setupEventListeners = useCallback(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      if (e.key === "Escape" || e.key === "Esc") {
        ...
        // Show warning modal
        // Can't prevent because of browse policies.
        // Do something here according the policies
        showModal({
      ...
      // Prevent Alt+Left/Right (browser back/forward navigation)
      ...
      // Prevent tab switching shortcuts
    ...
    // Record the change tab event
    const handleVisibilityChange = () => {
      const isVisible = document.visibilityState === "visible";
      setIsTabVisible(isVisible);

      // If test is active and user switched back to this tab, ensure we're in fullscreen
      if (isVisible && testActive && !isFullScreen) {
        enterFullScreen();
      }

      // If test is active and user is switching away from this tab, show warning
      if (!isVisible && testActive) {
        // We can't prevent tab switching, but we can log it and show a warning when they return
        sessionStorage.setItem("tabSwitchAttempted", "true");
      }

      // When user returns to the tab, check if they attempted to switch tabs
      if (
        isVisible &&
        ...
        // Show warning
        // Do something else acording to the policies
        showModal({
    ...
    const handleFullScreenChange = () => {
      ...
      // If user exited fullscreen and test is active, prompt to re-enter fullscreen
  ...
  }, [testActive, isFullScreen]); // Simplified dependencies
  ...
  }, [enterFullScreen, setupEventListeners]);
  ...
  const startTestInFullScreen = useCallback(() => {
    ...
    if (!browserInfo) {
      showMessage({
        content: "Checking browser compatibility...",
        type: "loading",
      });
      return false;
    }
